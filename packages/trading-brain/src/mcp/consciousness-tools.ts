...
function registerConsciousnessToolsWithCaller(server: McpServer, call: BrainCall): void {
  ...
  server.tool(
    'trading_consciousness_status',
    'Get Trading Brain consciousness status: total thoughts, active engines, thought breakdown, and live dashboard client count.',
    {},
    async () => {
      const status: AnyResult = await call('consciousness.status', {});
      const lines = [
        'Trading Brain Consciousness Status:',
  ...
  server.tool(
    'trading_consciousness_thoughts',
    'Get recent thoughts from Trading Brain consciousness stream. Shows what Trading Brain is thinking.',
    {
      engine: z.string().optional().describe('Filter by engine (e.g., "dream", "anomaly_detective")'),
    ...
    async (params) => {
      ...
      if (!thoughts?.length) return textResult('No thoughts yet. Trading Brain is quiet.');
