...
function registerCuriosityToolsWithCaller(server: McpServer, call: BrainCall): void {
  ...
  server.tool(
    'trading_curiosity_status',
    'Get Trading Brain curiosity status: knowledge gaps, unanswered questions, exploration rate, top bandit arms.',
    {},
    async () => {
      const status: AnyResult = await call('curiosity.status', {});
      const lines = [
        '# Trading Curiosity Status',
        '',
  ...
  server.tool(
    'trading_knowledge_gaps',
    'List Trading Brain knowledge gaps: topics with high attention but low knowledge.',
  ...
  server.tool(
    'trading_explore_next',
    'Ask Trading Brain what to explore next using UCB1 multi-armed bandit.',
    {},
    async () => {
  ...
  server.tool(
    'trading_surprises',
    'Detect surprises in Trading Brain: things that violated expectations.',
    {},
    async () => {
