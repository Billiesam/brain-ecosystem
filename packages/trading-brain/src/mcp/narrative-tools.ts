...
function registerNarrativeToolsWithCaller(server: McpServer, call: BrainCall): void {
  ...
  server.tool(
    'trading_explain',
    'Ask Trading Brain to explain what it knows about a topic. Returns principles, hypotheses, experiments, journal entries, and predictions — composed into a natural language narrative with confidence scores.',
    {
      topic: z.string().describe('The topic to explain (e.g. "win rate patterns", "DCA strategies", "risk management")'),
    },
    async (params) => {
  ...
  server.tool(
    'trading_ask',
    'Ask Trading Brain a natural language question. Brain searches across its knowledge base (principles, hypotheses, journal, experiments) and composes an answer. Supports German and English.',
    {
      question: z.string().describe('The question to ask (e.g. "why do losses cluster on Mondays?", "which signals have the best win rate?")'),
    },
    async (params) => {
  ...
  server.tool(
    'trading_weekly_digest',
    'Generate a weekly digest — a comprehensive Markdown report of what Trading Brain learned, discovered, predicted, and experimented with in the past N days.',
    {
  ...
  server.tool(
    'trading_contradictions',
    'Find contradictions in Trading Brain\'s knowledge — hypotheses vs anti-patterns, conflicting principles, failed predictions. Shows trade-offs and severity.',
    {},
    async () => {
      const contradictions: AnyResult[] = await call('narrative.contradictions', {}) as AnyResult[];
      if (!contradictions?.length) return textResult('No contradictions found. Trading Brain\'s knowledge is internally consistent.');
