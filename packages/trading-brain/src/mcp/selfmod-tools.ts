...
function registerSelfmodToolsWithCaller(server: McpServer, call: BrainCall): void {
  server.tool(
    'trading_selfmod_status',
    'Get the trading-brain self-modification engine status — counts of proposed, testing, ready, applied, failed modifications.',
    {},
    async () => {
      const status = await call('selfmod.status') as Record<string, unknown>;
      const byStatus = status.byStatus as Record<string, number> || {};
      const lines = [
        '## Trading-Brain Self-Modification Status',
  ...
  server.tool(
    'trading_selfmod_pending',
    'List all pending trading-brain self-modifications waiting for review — shows proposed and ready-for-approval code changes.',
    {},
    async () => {
  ...
  server.tool(
    'trading_selfmod_approve',
    'Approve a trading-brain self-modification and apply the code changes — only works for modifications with status "ready".',
  ...
  server.tool(
    'trading_selfmod_history',
    'Get the history of all trading-brain self-modifications — shows what changes the trading-brain has proposed, tested, applied, and rejected.',
