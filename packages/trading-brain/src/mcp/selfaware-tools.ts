...
function registerSelfawareToolsWithCaller(server: McpServer, call: BrainCall): void {
  ...
  server.tool(
    'trading_blind_spots',
    'Detect Trading Brain knowledge blind spots: areas with high activity but low understanding.',
    {},
    async () => {
      try {
        const result: AnyResult = await call('blindspot.detect', {});
        const lines = ['# Trading Blind Spots', ''];
  ...
  server.tool(
    'trading_meta_trends',
    'Get Trading Brain meta-learning trends: learning speed, hypothesis quality, experiment success rates.',
    ...
    async (params) => {
      try {
        ...
        const lines = ['# Trading Meta-Learning Trends', ''];
  ...
  server.tool(
    'trading_long_term_analysis',
    'Get Trading Brain long-term learning analysis: growth trajectory, milestones, learning velocity.',
    ...
    async (params) => {
      try {
        ...
        const lines = ['# Trading Long-Term Analysis', ''];
  ...
  server.tool(
    'trading_creative_hypotheses',
    'Generate creative trading hypotheses by combining unrelated domains, inverting assumptions, or applying analogies.',
    ...
    async (params) => {
      try {
        ...
        const lines = [`# Creative Trading Hypotheses: ${hypotheses.length}`, ''];
  ...
  server.tool(
    ...
    'Challenge a trading principle using Advocatus Diaboli: find counter-arguments and edge cases.',
  ...
  server.tool(
    'trading_dream_retrospective',
    'Analyze Trading Brain dream pruning: what was pruned, compressed, or strengthened, and whether decisions were good.',
    {},
    async () => {
      try {
        const result: AnyResult = await call('dream.retrospective', {});
        const lines = ['# Trading Dream Retrospective', ''];
        if (result) {
  ...
  server.tool(
    'trading_cross_brain_dialogue',
    'Start cross-brain dialogue from Trading Brain perspective on a topic.',
  ...
  server.tool(
    'trading_self_test',
    'Self-test all Trading Brain principles: verify against recent evidence, flag contradictions.',
    {},
    async () => {
      try {
        const result: AnyResult = await call('selftest.all', {});
        const lines = ['# Trading Self-Test Results', ''];
        if (result) {
  ...
  server.tool(
    'trading_self_test_report',
    'Get Trading Brain understanding report: what it knows, confidence levels, weakest areas.',
    {},
    async () => {
      try {
        const result: AnyResult = await call('selftest.report', {});
        const lines = ['# Trading Understanding Report', ''];
        if (result) {
  ...
  server.tool(
    'trading_teach_create',
    'Create a teaching package from Trading Brain knowledge for another brain.',
    { targetBrain: z.string().describe('Target brain (e.g. "brain", "marketing")') },
    async (params) => {
      try {
        const result: AnyResult = await call('teach.create', { targetBrain: params.targetBrain });
        const lines = [`# Trading Teaching Package → ${params.targetBrain}`, ''];
        if (result) {
  ...
  server.tool(
    'trading_teach_packages',
    'List Trading Brain teaching packages.',
    ...
    async (params) => {
      try {
        ...
        const lines = [`# Trading Teaching Packages: ${packages.length}`, ''];
  ...
  server.tool(
    'trading_scout_status',
    'Get Trading Brain DataScout status: monitored sources, scan times, discovery counts.',
    {},
    async () => {
      try {
        const result: AnyResult = await call('scout.status', {});
        const lines = ['# Trading DataScout Status', ''];
        if (result) {
  ...
  server.tool(
    'trading_scout_discoveries',
    'Get Trading Brain scout discoveries from monitored sources.',
    {
  ...
  server.tool(
    'trading_simulate',
    'Run what-if simulation on Trading Brain: predict effects of metric changes.',
    {
      metric: z.string().describe('Metric to simulate (e.g. "win_rate", "drawdown")'),
    ...
    async (params) => {
      try {
        ...
        const lines = [`# Trading Simulation: ${params.metric} x${params.multiplier}`, ''];
  ...
  server.tool(
    'trading_emergence_explain',
    'Explain an emergence event in Trading Brain: why it occurred and what it means.',
    { eventId: z.number().describe('Emergence event ID') },
    async (params) => {
      try {
        const result: AnyResult = await call('emergence.explain', { eventId: params.eventId });
        const lines = ['# Trading Emergence Explanation', ''];
        if (result?.event) {
