...
function registerPeerToolsWithCaller(server: McpServer, call: BrainCall): void {
  ...
  server.tool(
    'brain_peer_status',
    'Get Brain peer network status: discovery state, online/offline peers, knowledge summaries',
    {},
    async () => {
  ...
  server.tool(
    'brain_peer_list',
    'List all discovered Brain peers with connection details',
    {},
    async () => {
      const peers: AnyResult = await call('peer.list', {});
      const lines = ['# Discovered Peers', ''];
      if (peers?.length > 0) {
