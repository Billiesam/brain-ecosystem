...
function registerTransferToolsWithCaller(server: McpServer, call: BrainCall): void {
  ...
  server.tool(
    'marketing_transfer_status',
    'Get cross-domain knowledge transfer status for Marketing Brain: analogies, transfers, rules, effectiveness.',
    {},
    async () => {
      const status: AnyResult = await call('transfer.status', {});
      const lines = [
        'Transfer Engine Status (Marketing):',
  ...
  server.tool(
    'marketing_transfer_analogies',
    'Find cross-domain analogies relevant to marketing — patterns similar across Brain domains.',
  ...
  server.tool(
    'marketing_transfer_rules',
    'List cross-domain rules affecting Marketing Brain.',
    {},
    async () => {
