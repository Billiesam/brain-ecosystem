...
function registerNarrativeToolsWithCaller(server: McpServer, call: BrainCall): void {
  ...
  server.tool(
    'marketing_explain',
    'Ask Marketing Brain to explain what it knows about a topic. Returns principles, hypotheses, experiments, journal entries, and predictions — composed into a natural language narrative with confidence scores.',
    {
      topic: z.string().describe('The topic to explain (e.g. "engagement patterns", "content strategies", "audience growth")'),
    },
    async (params) => {
  ...
  server.tool(
    'marketing_ask',
    'Ask Marketing Brain a natural language question. Brain searches across its knowledge base (principles, hypotheses, journal, experiments) and composes an answer. Supports German and English.',
    {
      question: z.string().describe('The question to ask (e.g. "which platforms drive most engagement?", "what content types perform best?")'),
    },
    async (params) => {
  ...
  server.tool(
    ...
    'Generate a weekly digest — a comprehensive Markdown report of what Marketing Brain learned, discovered, predicted, and experimented with in the past N days.',
    {
  ...
  server.tool(
    'marketing_contradictions',
    'Find contradictions in Marketing Brain\'s knowledge — hypotheses vs anti-patterns, conflicting principles, failed predictions. Shows trade-offs and severity.',
    {},
    async () => {
      const contradictions: AnyResult[] = await call('narrative.contradictions', {}) as AnyResult[];
      if (!contradictions?.length) return textResult('No contradictions found. Marketing Brain\'s knowledge is internally consistent.');
