...
function registerDebateToolsWithCaller(server: McpServer, call: BrainCall): void {
  ...
  server.tool(
    'marketing_debate_start',
    'Start a multi-perspective debate on a question. Marketing Brain generates its perspective from principles, hypotheses, journal, anomalies, and predictions.',
    ...
    async (params) => {
      ...
      lines.push(`*Add more perspectives from other brains, then call marketing_debate_synthesize with debateId=${debate.id}*`);
      return textResult(lines.join('\n'));
    },
  );

  server.tool(
    'marketing_debate_synthesize',
  ...
  server.tool(
    'marketing_debate_perspective',
    'Generate Marketing Brain\'s perspective on a question without starting a full debate. Useful for cross-brain debates.',
  ...
  server.tool(
    'marketing_debate_history',
    'View past Marketing Brain debates with their questions, perspectives, synthesis, and conflicts.',
    ...
    async (params) => {
      ...
      if (!debates?.length) return textResult('No debates yet. Start one with marketing_debate_start.');
