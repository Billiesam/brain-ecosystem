...
function registerConsciousnessToolsWithCaller(server: McpServer, call: BrainCall): void {
  ...
  server.tool(
    'marketing_consciousness_status',
    'Get Marketing Brain consciousness status: total thoughts, active engines, thought breakdown, and live dashboard client count.',
    {},
    async () => {
      const status: AnyResult = await call('consciousness.status', {});
      const lines = [
        'Marketing Brain Consciousness Status:',
  ...
  server.tool(
    'marketing_consciousness_thoughts',
    'Get recent thoughts from Marketing Brain consciousness stream. Shows what Marketing Brain is thinking.',
    {
      engine: z.string().optional().describe('Filter by engine (e.g., "dream", "cross_domain")'),
    ...
    async (params) => {
      ...
      if (!thoughts?.length) return textResult('No thoughts yet. Marketing Brain is quiet.');
