...
function registerPeerToolsWithCaller(server: McpServer, call: BrainCall): void {
  ...
  server.tool(
    'marketing_peer_status',
    'Get Marketing Brain peer network status: discovery state, online/offline peers, knowledge summaries',
    {},
    async () => {
      const status: AnyResult = await call('peer.status', {});
      const lines = [
        '# Marketing Peer Network Status',
  ...
  server.tool(
    'marketing_peer_list',
    'List all discovered Marketing Brain peers with connection details',
    {},
    async () => {
      const peers: AnyResult = await call('peer.list', {});
      const lines = ['# Marketing Discovered Peers', ''];
      if (peers?.length > 0) {
