...
function registerDreamToolsWithCaller(server: McpServer, call: BrainCall): void {
  ...
  server.tool(
    'marketing_dream_status',
    'Get Marketing Brain Dream Mode status: whether dreaming is active, total cycles, lifetime consolidation stats.',
    {},
    async () => {
      const status: AnyResult = await call('dream.status', {});
      const lines = [
        'Marketing Brain Dream Mode Status:',
  ...
  server.tool(
    'marketing_dream_consolidate',
    'Manually trigger a Marketing Brain Dream Mode consolidation cycle. Replays content memories, prunes weak synapses, compresses similar patterns.',
    {
  ...
  server.tool(
    'marketing_dream_history',
    'Get past Marketing Brain Dream Mode cycles. Shows consolidation stats per cycle.',
    {
    ...
    async (params) => {
      ...
      const lines = [`Marketing Brain Dream History (${history.length} cycles):\n`];
